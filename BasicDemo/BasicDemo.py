...
if __name__ == "__main__":
    ...
    def enum_devices():
        ...
        for i in range(0, deviceList.nDeviceNum):
            ...
            if mvcc_dev_info.nTLayerType == MV_GIGE_DEVICE or mvcc_dev_info.nTLayerType == MV_GENTL_GIGE_DEVICE:
            ...
            elif mvcc_dev_info.nTLayerType == MV_USB_DEVICE:   # USB
                print("\nUSB3.0 Camera: [%d]" % i)
                ...
                #                + "(" + str(strSerialNumber) + ")")
                devList.append("[" + str(i) + "]USB3.0协议: " " " + model_name)
            elif mvcc_dev_info.nTLayerType == MV_GENTL_CAMERALINK_DEVICE:   # CameraLink
                print("\nCML device: [%d]" % i)
                ...
                devList.append("[" + str(i) + "]CML: " + user_defined_name + " " + model_name
                               + "(" + str(strSerialNumber) + ")")
            elif mvcc_dev_info.nTLayerType == MV_GENTL_CXP_DEVICE:   # CXP
                print("\nCXP device: [%d]" % i)
                ...
                devList.append("[" + str(i) + "]CXP: " + user_defined_name + " " + model_name
                               + "(" + str(strSerialNumber) + ")")
            elif mvcc_dev_info.nTLayerType == MV_GENTL_XOF_DEVICE:   # XoF
                print("\nXoF device: [%d]" % i)
    ...
    def open_device():
        ui.bnSetLightOff.setEnabled(True)
        ui.bnSetLightHalf.setEnabled(True)
        ui.bnSetLightFull.setEnabled(True)
        global deviceList
        global nSelCamIndex
        global obj_cam_operation
        global isOpen
    ...
    def start_grabbing():
        global obj_cam_operation
        global isGrabbing
        ui.bnSavePathClear.setEnabled(False)
        ret = obj_cam_operation.Start_grabbing(ui.widgetDisplay.winId())
        # ret = obj_cam_operation.Start_grabbing(ui.graphicsView)
        if ret != 0:
            ...
            isGrabbing = True
            enable_controls()
            ui.Line_FileName.setText(str(count))
    ...
    def select_save_path(self):
        global output_dir, image_save_path
        desktop_path = QStandardPaths.writableLocation(QStandardPaths.DesktopLocation)
        output_dir = QFileDialog.getExistingDirectory(None, "选择目录", desktop_path, QFileDialog.ShowDirsOnly)
        name_length = len(output_dir)
        if name_length == 0:
            ui.SavePath.append("请重新选择")
        else:
            ui.SavePath.append(output_dir)
            output_dir = output_dir + '/'
            image_save_path_update()
            ui.bnSavePathClear.setEnabled(True)
    ...
    def clear_count():
        ...
        count = 1
        ui.Line_FileName.setText(str(count))
    def image_save_path_update():
        global image_save_path
        image_save_path = output_dir + ui.Line_FileName.text()



    # ch:存图 | en:save image
    def save_bmp():
        global count,totle_count
        image_save_path_update()
        ret = obj_cam_operation.Save_Bmp(image_save_path)
        if ret != MV_OK:
            ...
            totle_count += 1
            ui.Line_FileName.setText(str(count))
            ui.ImageCounter.clear()
            ui.ImageCounter.append("累计保存：" + str(totle_count))
    def save_jpg():
        global count,totle_count
        image_save_path_update()
        ret = obj_cam_operation.Save_jpg(image_save_path)
        if ret != MV_OK:
            strError = "Save JPG failed ret:" + ToHexStr0(ret)
            QMessageBox.warning(mainWindow, "Error", strError, QMessageBox.Ok)
        else:
    ...
    def get_param():
        ...
        if ret != MV_OK:
            ...
            ui.edtExposureTime.setText("{0:.0f}".format(obj_cam_operation.exposure_time))
            ui.edtGain.setText("{0:.2f}".format(obj_cam_operation.gain))
            ui.edtFrameRate.setText("{0:.2f}".format(obj_cam_operation.frame_rate))
            if (1000000/obj_cam_operation.exposure_time)>19:
                ReadFrameRate = 19
            else:
                ReadFrameRate = 1000000/obj_cam_operation.exposure_time
            ui.label_11.setText("{0:.1f}".format(ReadFrameRate))
            # ui.label_3.setText("{0:.1f}".format(1000000/obj_cam_operation.exposure_time))
    ...
    def set_param():
        ...
        if is_float(frame_rate)!=True or is_float(exposure)!=True or is_float(gain)!=True:
            ...
            strError = "参数非法！应该是超出范围了"
            QMessageBox.warning(mainWindow, "Error 错误", strError, QMessageBox.Ok)
    ...
    def enable_controls():
        ...
        ui.bnSavePath.setEnabled(isOpen and isGrabbing)

        ui.bnSavePathClear.setEnabled(isOpen and isGrabbing)
    ...
    mainWindow.setWindowTitle('Hikvision Camera Snapshot')
    ...
    ui.bnSavePath.clicked.connect(select_save_path)
    ui.bnSavePathClear.clicked.connect(clear_count)
    ...
    ui.bnSaveImage_2.clicked.connect(save_jpg)
